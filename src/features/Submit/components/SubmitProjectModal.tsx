...
const SubmitProjectModal: React.FC<SubmitProjectModalProps> = ({ isOpen, onClose }) => {
    ...
    const renderProjectStep = () => {
        switch (currentStep) {
            case 1:
                return <StepEssentials data={projectData} updateData={updateProjectData} />;
            case 2:
                return <StepCategorization data={projectData} updateData={updateProjectData} />;
            case 3:
                return <StepVisuals data={projectData} updateData={updateProjectData} />;
            case 4:
                return (
                    <div className="flex flex-col gap-4 text-center py-8">
                        ...
                        <h3 className="text-xl font-bold text-textColor">Ready to Submit?</h3>
                        <p className="text-textColorWeak">
                            Please review your information before submitting. Your project will be reviewed by our team shortly.
                        </p>
                        <div className="bg-cardItemBg rounded-xl p-4 text-left mt-4 text-sm text-textColorWeak">
                            <p><strong className="text-textColor">Name:</strong> {projectData.name}</p>
                            <p><strong className="text-textColor">Tagline:</strong> {projectData.tagline}</p>
                            <p><strong className="text-textColor">Categories:</strong> {projectData.categories.join(', ')}</p>
                        </div>
                    </div>
                );
            default:
    ...
    const getTitle = () => {
        switch (view) {
            case 'selection': return 'Submit Content';
            case 'project': return 'Submit Project';
            case 'category': return 'Suggest Category';
            case 'template': return 'Submit Template';
        }
    };
                    ...
                    >
                        ...
                        <div className="flex items-center justify-between p-6 border-b border-linesColor">
                            <div className="flex items-center gap-3">
                                {view !== 'selection' && (
                                    <button
                                        onClick={() => view === 'project' ? handleBack() : setView('selection')}
                                ...
                                <div>
                                    <h2 className="text-xl font-bold text-textColor">{getTitle()}</h2>
                                    {view === 'project' && (
                                        <p className="text-sm text-textColorWeak">Step {currentStep} of {totalProjectSteps}</p>
                                    )}
                                </div>
                            </div>
                            <button
                        ...
                        {view === 'project' && (
                            <div className="w-full h-1 bg-cardItemBg">
                                <motion.div
                                    ...
                                    initial={{ width: `${((currentStep - 1) / totalProjectSteps) * 100}%` }}
                                    animate={{ width: `${(currentStep / totalProjectSteps) * 100}%` }}
                        ...
                        <div className="p-6 overflow-y-auto flex-1">
                            ...
                            {view === 'category' && <StepCategoryForm onSubmit={handleCategorySubmit} />}
                            {view === 'template' && (
                        ...
                        {view === 'project' && (
                            <div className="p-6 border-t border-linesColor flex justify-between items-center bg-bodyBg">
                                ...
                                >
                                    {currentStep === totalProjectSteps ? 'Submit Project' : 'Continue'}
                                </button>
                            </div>
                        )}
                    </motion.div>
